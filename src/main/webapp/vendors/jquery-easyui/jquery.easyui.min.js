...
(function ($) {
    ...
    function _21d(target) {
        var states = $.data(target, "panel");
        var opts = states.options;
        var _panel = states.panel;
        _panel.css(opts.style);
        _panel.addClass(opts.cls);
        _221();
        _222();
        var _header = $(target).panel("header");
        var body = $(target).panel("body");
        var _footer = $(target).siblings(".panel-footer");
        if (opts.border) {
            _header.removeClass("panel-header-noborder");
            body.removeClass("panel-body-noborder");
            _footer.removeClass("panel-footer-noborder");
        } else {
            _header.addClass("panel-header-noborder");
            body.addClass("panel-body-noborder");
            _footer.addClass("panel-footer-noborder");
        }
        _header.addClass(opts.headerCls);
        ...
        function _221() {
            if (opts.noheader || (!opts.title && !opts.header)) {
                _20b(_panel.children(".panel-header"));
                _panel.children(".panel-body").addClass("panel-body-noheader");
            } else {
                if (opts.header) {
                    // TODO 如果opts.header中存在英文()字符，则下面的代码会报错。
                    // 注释掉代码后，暂时未发现easyui运行有何不妥
                    // 如果下面的注释发现影响easyui的功能，则需要在这里将opts.header中的英文括号字符删除或者转义为中文括号字符。
                    //$(opts.header).addClass("panel-header").prependTo(_panel);
                } else{
                    var _225 = _panel.children(".panel-header");
                    if (!_225.length) {
                        _225 = $("<div class=\"panel-header\"></div>").prependTo(_panel);
                    }
        ...
        function _222() {
            if (opts.footer) {
                $(opts.footer).addClass("panel-footer").appendTo(_panel);
                $(target).addClass("panel-body-nobottom");
            } else {
                _panel.children(".panel-footer").remove();
